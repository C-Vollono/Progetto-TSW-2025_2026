...
<section class="catalog-section" style="padding: 20px; max-width: 1400px; margin: 0 auto;">
	
	<div class="catalog-header" style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #eaeaea; padding-bottom: 20px; margin-bottom: 30px;">
		<div style="text-align: left; margin: 0;">
			<h2 style="margin: 0; font-size: 2.2rem; font-weight: bold; color: #222;">Gestione Catalogo <span style="color: #b5832e;">Premium</span></h2>
		</div>
		<a href="${pageContext.request.contextPath}/jsp/admin/inserisciProdottoForm.jsp" class="btn-add-product">
			+ Inserisci Nuovo Prodotto
		</a>
	</div>
	
	<c:if test="${not empty sessionScope.messaggioSuccesso}">
		<div style="background-color: #d4edda; color: #155724; padding: 12px; margin-bottom: 20px; border-radius: 4px; font-weight: bold; font-family: Arial, sans-serif;">
	...
	<c:if test="${not empty sessionScope.messaggioErrore}">
		<div style="background-color: #f8d7da; color: #721c24; padding: 12px; margin-bottom: 20px; border-radius: 4px; font-weight: bold; font-family: Arial, sans-serif;">
				...
				<c:choose>
					<c:when test="${empty prodottiCatalogo}">
						<div class="empty-catalog-msg">
							<p>Nessun prodotto corrisponde ai criteri di ricerca cercati nel catalogo admin.</p>
						</div>
					</c:when>
							...
							<div class="product-card">
								...
								<div class="product-info">
									...
									<p class="product-price">€ ${prodotto.prezzo}</p>
										...
										<a href="${pageContext.request.contextPath}/Admin/GestioneProdotti?action=editForm&idProdotto=${prodotto.idProdotto}" class="btn-details" style="background-color: #333; color: #fff; text-align: center; flex: 1; text-decoration: none; padding: 10px 0; font-size: 0.9rem; font-weight: bold; border-radius: 4px;">
											Modifica
										</a>
										
										<form action="${pageContext.request.contextPath}/Admin/GestioneProdotti" method="POST" style="flex: 1; margin: 0;" onsubmit="return confirm('Sei sicuro di voler eliminare permanentemente questo prodotto dal catalogo?');">
											<input type="hidden" name="action" value="delete">
											<input type="hidden" name="idProdotto" value="${prodotto.idProdotto}">
											<button type="submit" style="width: 100%; background-color: #ff4d4d; color: white; border: none; padding: 10px 0; font-size: 0.9rem; font-weight: bold; border-radius: 4px; cursor: pointer; transition: background-color 0.2s;">
												Elimina
											</button>
										</form>
